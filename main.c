...
// Function to test the A* algorithm for the road network
void testAStarAlgorithm() {
    ...
    // Time the A* algorithm
    startTimer();
    int approxDist = approximateSolution(network, start, end);
    printf("A* Algorithm Approximate Distance: %d\n", approxDist);
    printf("A* Algorithm took %lf seconds.\n", getTimeElapsed());
...
int main() {
    ...
    // Time the A* algorithm on the road network
    testAStarAlgorithm();
    ...
    printf("\nRunning Dijkstra tests on graph with no possible path:\n");
    testDijkstraNoPath();

    printf("\nRunning Dijkstra tests on single-node graph:\n");
